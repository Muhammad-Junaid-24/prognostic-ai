...
- add `.env.example` files for both `api-prognostic` and `web-app-prognostic` with required env keys,
- generate a `docker-compose.yml` for local MySQL + Redis,
- or add a root-level script to install deps for both subprojects.

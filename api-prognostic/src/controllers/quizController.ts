...
export const createWebScanQuizWithTypeForm = async (
  ...
  try {
    ...
    const typeFormId = typeformResponse.id;
    const typeformEmbedUrl = typeformResponse._links.display;

    await createTypeformWebhook(typeFormId);

    newCampaign.typeFormId = typeFormId;
    newCampaign.typeFormEmbeddedLink = typeformEmbedUrl;
    await newCampaign.save({ transaction });

    const sanitizedCompanyName = primaryCompany.companyName.replace(/\s+/g, '-');
    ...
    <div class="content">
    <div data-tf-widget="${typeFormId}" style="width: 100%; height: 100%;"></div>
    </div>
    </div>
    <div class="footer">
    <a href="https://app.clients.ai" class="clients-button" target="_blank">Clients.ai Enabled</a>
    </div>
    </div>
...
`;
    ...
    const clientsAiUrl = `http://app.clients.ai/form/${sanitizedCompanyName}-ClientsAi?${queryParams}`;
